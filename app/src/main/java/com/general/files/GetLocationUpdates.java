...
//                        Utils.printELog("RESOLUTION_REQUIRED","::"+statusCode);
                        switch (statusCode) {
                            case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                                try {
                                    // Show the dialog by calling startResolutionForResult(), and check the
                                    // result in onActivityResult().
                                    if (isInitializeFromNonActivityClass == false && (mContext instanceof KioskLandingScreenActivity)) {
...
//                                    Utils.printELog("Error", "PendingIntent unable to execute request.");
                                }
                                break;
                        }
                    });
    ...
    public void onLocationResult(LocationResult locationResult) {
        ...
        if (Utils.SKIP_MOCK_LOCATION_CHECK && mCurrentLocation == null)
        {
            return;
        }
        else  if (!Utils.SKIP_MOCK_LOCATION_CHECK && (mCurrentLocation == null || (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2 && mCurrentLocation.isFromMockProvider()) || (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2 && isMockSettingsON(mContext)))) {
